        ...
        for (int submeshIterator = 0; submeshIterator < victimMesh.subMeshCount; submeshIterator++)
        {
            ...
            for (int i = 0; i < indices.Length; i += 3)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    triangleCache.vertices[j] = mesh_vertices[index[j]];
                    triangleCache.normals[j] = mesh_normals[index[j]];
                    triangleCache.uvs[j] = mesh_uvs[index[j]];

                    if(mesh_tangents != null)
                    {
                        triangleCache.tangents[j] = mesh_tangents[index[j]];
                    }
                    else
                    {
                        triangleCache.tangents[j] = Vector4.zero;
                    }
